...
with tab_macro:
    ...
    nombres_boxes = ["Box 1", "Box 2", "Box 3", "Box 4", "Box 5 (Aislado)"]

    # Fila 1 de Camas (3 Cajas)
    cols_fila1 = st.columns(3)
    for i in range(3):
        if i < len(pacientes_lista):
            nombre_p, datos = pacientes_lista[i]
            ...
            with cols_fila1[i]:
                ...
                <div style="background-color: #1C1E24; border: 2px solid {color_box}; border-radius: 10px; padding: 15px; box-shadow: 0 4px 8px rgba(0,0,0,0.3); margin-bottom: 20px;">
                    <div style="background-color: {color_box}; padding: 5px; border-radius: 5px; margin-bottom: 10px; text-align: center;">
                        <h4 style="color: white; margin: 0;">{nombres_boxes[i]}</h4>
                    </div>
    ...
    # Fila 2 de Camas (2 Cajas Centradas)
    cols_fila2 = st.columns([0.5, 1, 1, 0.5])
    for i in range(3, 5):
        if i < len(pacientes_lista):
            nombre_p, datos = pacientes_lista[i]
            color_box = obtener_color_cama(datos)
