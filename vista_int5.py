...
with tab_macro:
    ...
    with sub_tab_camas:
        ...
        def obtener_color_cama(datos_paciente):
            ...
            tiene_alarma_critica = any(nivel == "CRÍTICO" for nivel, _ in alarmas)
            tiene_advertencia = len(alarmas) > 0

            if news2 >= 7 or tiene_alarma_critica:
                return "#c0392b"  # Rojo (Crítico)
            elif news2 >= 5 or parametro_extremo or tiene_advertencia:
                return "#f39c12"  # Naranja (Urgente / Advertencia activa)
            else:
                return "#2ca02c"  # Verde (Estable)
