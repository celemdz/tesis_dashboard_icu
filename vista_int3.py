...
# ==============================================================================
# 1. CONFIGURACION DE LA PAGINA Y ESTILOS GLOBALES
# ==============================================================================
st.set_page_config(layout="wide", page_title="Central de Monitoreo UCI")

# Inyección de CSS para agrandar el texto de las pestañas principales
st.markdown("""
<style>
...
# ==============================================================================
# 2. EXTRACCION DE LA BASE DE DATOS REAL (15 CAMAS)
# ==============================================================================
@st.cache_data
def cargar_macro():
...
def generar_perfiles_dinamicos():
    ...
    stays_objetivo = df_icu['stay_id'].dropna().unique()[:15] # Aumentado a 15 camas
    ...
    for stay in stays_objetivo:
        ...
        perfiles[f"Paciente MIMIC-IV (Stay ID: {stay})"] = {
...
# Creación de las dos grandes pestañas
...
with tab_macro:
    ...
    with sub_tab_camas:
        st.markdown("Mapa topográfico de 15 camas. El color indica la prioridad clínica en tiempo real (NEWS2).")
        st.write("")
        ...
        # Bucle de 3 filas x 5 columnas para 15 camas
        for fila in range(3):
            cols = st.columns(5)
                    ...
                    with cols[col_idx]:
                        ...
                        <div style="background-color: #1C1E24; border: 2px solid {color_box}; border-radius: 10px; padding: 15px; box-shadow: 0 4px 8px rgba(0,0,0,0.3); margin-bottom: 20px;">
                            ...
                            <div style="display: flex; justify-content: space-around;">
                                <div style="text-align: center;"><p style="color: #00E676; margin: 0; font-size: 12px; font-weight: bold;">FC</p><h4 style="color: white; margin: 0;">{int(datos['fc_base'])}</h4></div>
                                <div style="text-align: center;"><p style="color: #FF4B4B; margin: 0; font-size: 12px; font-weight: bold;">PAM</p><h4 style="color: white; margin: 0;">{int(datos['pam'])}</h4></div>
                                <div style="text-align: center;"><p style="color: #00B0FF; margin: 0; font-size: 12px; font-weight: bold;">SpO2</p><h4 style="color: white; margin: 0;">{int(datos['spo2_base'])}</h4></div>
                            </div>
                        </div>
                        """)

    with sub_tab_kpis:
        st.markdown("### El Desarrollo de un Dashboard Clínico para el Monitoreo de KPIs")
...
# ==============================================================================
with tab_micro:
    st.title(f" VISTA MICRO: Monitorización Continua ({paciente_global})")
    st.write("")
